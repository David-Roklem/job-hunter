/**
 * CLI сбора вакансий с Wellfound.
 *
 * Запуск:
 ...
 * печатает статистику. Требует (опционально) предварительный логин
 * (npm run wellfound:login) — публичные listings частично работают и без него.
 */
import { loadEnv } from "./_env";

loadEnv();

const { collectVacancies } = await import("../app/wellfound/collect");
...
function parseArgs(argv: string[]): {
  ...
  for (const arg of argv.slice(2)) {
    if (arg.startsWith("--source=")) out.source = Number(arg.split("=")[1]);
      ...
      out.profile = Number(arg.split("=")[1]);
...
async function main(): Promise<void> {
  ...
  if (args.source === undefined || args.profile === undefined) {
    console.error(
      "Использование: npm run wellfound:collect -- --source=<id> --profile=<id> [--max=<n>] [--headed]",
    );
    console.error("  --source   id источника в БД (kind=aggregator)");
  ...
  console.log("=== Wellfound collect ===\n");
  console.log(
    `source=${args.source}, profile=${args.profile}, max=${args.max ?? 20}\n`,
  );
  ...
  try {
    ...
    if (stats.blocked) {
      console.log(
        "  ⚠ анти-бот блокировка — повторите логин (npm run wellfound:login) или снизьте частоту",
      );
    }
  } catch (err) {
    console.error("\n✗ Ошибка:", err);
